...
async function enviarConvite() {
  ...
  try {
    const convite = ehProfissional.value
...
async function criarNovaConta() {
  ...
  try {
    ...
    if (ehProfissional.value) {
      ...
      const convite = await conviteService.criarConviteProfissional(estabelecimentoId.value, {
        email: emailTrim,
        telefone: telefoneTrim || undefined,
...
<template>
  <div class="mx-auto max-w-lg space-y-4 lg:space-y-6">
    ...
    <BaseAlert v-if="modo === 'convite'" variant="info">
      Informe o <strong>e-mail</strong> e a função. Você recebe um link para copiar e enviar
      (WhatsApp, e-mail, etc.). Só a conta com esse e-mail pode aceitar.
    </BaseAlert>
